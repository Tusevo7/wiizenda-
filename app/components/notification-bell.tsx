...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    async function initNotifications() {
      ...
      // 1. Carregar notificações existentes
      ...
      // 2. Configurar Realtime (.on DEVE vir estritamente ANTES de .subscribe)
      channel = supabase
  ...
  function getNotificationIcon(notification: Notification) {
    if (notification.type === 'booking_confirmed') {
      return <CheckCircle2 size={19} className="text-green-600" />
    }
    ...
    if (notification.type === 'booking_new' || notification.booking_id) {
      return <CalendarCheck size={19} className="text-orange-500" />
    }
              ...
              {notifications.length > 0 ? (
                ...
                <div className="px-5 py-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    Quando acontecer algo importante, aparecerá aqui.
                  </p>
                </div>
              )}
            </div>
